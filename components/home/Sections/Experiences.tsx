...
const videos = [
  {
    id: 1,
    label: "Deluxe",
    title: "Every Stay is a Story to Remember",
    rating: 4,
    src: "/default.mp4",
...
export default function Experiences() {
  ...
  // Click animation refs
  const overlayTitleRef = useRef<HTMLHeadingElement | null>(null);
  const buttonRef = useRef<HTMLButtonElement | null>(null);

  /**
   * Scroll entrance animation (matches your heading section style)
   */
  useEffect(() => {
    ...
    if (!main || !header || !kicker || !title || !leftLine || !rightLine) {
      return;
    }
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from(
        title.querySelectorAll("span, br"),
        {
          y: 70,
          duration: 0.7,
          ease: "power3.out",
    ...
    return () => {
      ctx.revert(); // correct cleanup
    };
  }, []);

  /**
   * Click animation (unchanged, plays when active changes)
   */
  useEffect(() => {
    ...
    <section ref={mainRef} className={styles.section} data-cursor-theme="dark">
      <div className={styles.container}>
        ...
        <div className={styles.layout}>
          {/* Main Video */}
          <div className={`${styles.mainVideo} mainVideo`}>
            <video
              key={current.id}
              src={current.src}
              muted
